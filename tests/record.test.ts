...
import { GlowV3Event } from '../src/GlowV3Event';

describe('Event Record', () => {
    const contractConfig = buildtContracConfig()
    const glowV3Event = new GlowV3Event(currPublicClient)
    const pools: Address[] = ["0x200DA8F420Aec8223BB1cA57F80A0B132286FCF8"]

    test('DecreaseLiquidity getEventRecords', async () => {
        const datas = await glowV3Event.getEventRecords(pools, EventType.DecreaseLiquidity)
        console.log('datas: ', datas, datas.length)
    })

    test('IncreaseLiquidity getEventRecords', async () => {
        const datas = await glowV3Event.getEventRecords(pools, EventType.IncreaseLiquidity)
        console.log('datas: ', datas, datas.length)
    })

    test('Swap getEventRecords', async () => {
        const datas = await glowV3Event.getEventRecords(pools, EventType.Swap)
        console.log('datas: ', datas, datas.length)
    })

    test('MasterChefV3_AddPool getEventRecords', async () => {
        const datas = await glowV3Event.getEventRecords([contractConfig.MasterChefV3], EventType.MasterChefV3_AddPool, 0n)
        console.log('datas: ', datas, datas.length)
    })
