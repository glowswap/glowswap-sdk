...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    event AddPool(uint256 indexed pid, IGlowV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    function addRewarder(
        ...
        IGlowV3Pool _v3Pool = poolInfo[_pid].v3Pool;
    ...
    function getRewarderLength(IGlowV3Pool _v3Pool) external view returns (uint256) {
        if (address(_v3Pool) == address(0)) revert ZeroAddress();
        return poolInfo[v3PoolAddressPid[address(_v3Pool)]].rewarderList.length;
    }

    function getRewarderInfo(IGlowV3Pool _v3Pool, uint256 index) external view returns (PoolRewarderInfo memory) {
        if (address(_v3Pool) == address(0)) revert ZeroAddress();
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotGlowNFT();
    ...
    function getLatestPeriodInfoByPid(uint256 _pid, address _rewardToken)
        public
        view
        returns (uint256 rewardPerSecond, uint256 endTime)
    {
    ...
    function getLatestPeriodInfo(address _v3Pool, address _rewardToken)
        public
        view
        returns (uint256 rewardPerSecond, uint256 endTime)
    {
