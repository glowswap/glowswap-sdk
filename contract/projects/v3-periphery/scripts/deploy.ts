...
const artifacts: { [name: string]: ContractJson } = {
  ...
  GlowInterfaceMulticall: require('../artifacts/contracts/lens/GlowInterfaceMulticall.sol/GlowInterfaceMulticall.json'),
...
async function main() {
  ...
  const glowV3PoolDeployer_address = deployedContracts.GlowV3PoolDeployer
  const glowV3Factory_address = deployedContracts.GlowV3Factory
  ...
  const swapRouter = await SwapRouter.deploy(glowV3PoolDeployer_address, glowV3Factory_address, config.WNATIVE)
  ...
  const baseTokenUri = 'https://nft.glowprotocol.io/v3/'
  ...
  // GlowInterfaceMulticall
  const GlowInterfaceMulticall = new ContractFactory(
    artifacts.GlowInterfaceMulticall.abi,
    artifacts.GlowInterfaceMulticall.bytecode,
    owner
  )

  const glowInterfaceMulticall = await GlowInterfaceMulticall.deploy()
  console.log('GlowInterfaceMulticall deployted at:', glowInterfaceMulticall.address)
  await sleep(10000)
  ...
  const quoter = await Quoter.deploy(glowV3PoolDeployer_address, glowV3Factory_address, config.WNATIVE)
  ...
  const contracts = {
    ...
    GlowInterfaceMulticall: glowInterfaceMulticall.address,
  }
