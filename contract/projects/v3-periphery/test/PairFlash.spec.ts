...
import { MockTimeNonfungiblePositionManager, PairFlash, TestERC20, Quoter, IGlowV3Factory } from '../typechain-types'
  ...
  describe('flash', () => {
    it('test correct transfer events', async () => {
      ...
      const expectedAmountOut0 = await quoter.callStatic.quoteExactInputSingle({
        ...
        fee: FeeAmount.LOW,
        amountIn: amount1In,
        sqrtPriceLimitX96: encodePriceSqrt(20, 10),
      })
      const expectedAmountOut1 = await quoter.callStatic.quoteExactInputSingle({
        ...
        fee: FeeAmount.HIGH,
        amountIn: amount0In,
        sqrtPriceLimitX96: encodePriceSqrt(5, 10),
      })
      ...
      await expect(flash.initFlash(flashParams))
        ...
        .to.emit(token0, 'Transfer')
        .withArgs(pool2, flash.address, expectedAmountOut0.amountOut)
        .to.emit(token1, 'Transfer')
        .withArgs(pool3, flash.address, expectedAmountOut1.amountOut)
        .to.emit(token0, 'Transfer')
        .withArgs(flash.address, wallet.address, expectedAmountOut0.amountOut.toNumber() - amount0In - fee0)
        .to.emit(token1, 'Transfer')
        .withArgs(flash.address, wallet.address, expectedAmountOut1.amountOut.toNumber() - amount1In - fee1)
    })
