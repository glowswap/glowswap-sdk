...
import { TestGlowV3SwapPay } from '../typechain-types/contracts/test/TestGlowV3SwapPay'
...
import { TestGlowV3ReentrantCallee } from '../typechain-types/contracts/test/TestGlowV3ReentrantCallee'
...
describe('GlowV3Pool', () => {
  ...
  let factory: GlowV3Factory
  let pool: MockTimeGlowV3Pool

  let swapTarget: TestGlowV3Callee
  ...
  describe('#burn', () => {
    ...
    it('does not clear the position fee growth snapshot if no more liquidity', async () => {
      ...
      expect(feeGrowthInside0LastX128).to.eq('226854911280625642308916404954512140')
      expect(feeGrowthInside1LastX128).to.eq('226854911280625869163827685580154449')
    })
  ...
  async function initializeAtZeroTick(pool: MockTimeGlowV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('limit orders', () => {
    ...
    it('limit selling 0 for 1 at tick 0 thru 1', async () => {
      ...
      await expect(pool.collect(wallet.address, 0, 100, MaxUint128, MaxUint128))
        .to.emit(token1, 'Transfer')
        .withArgs(pool.address, wallet.address, BigNumber.from('5012269623051203').add('10049663404614')) // roughly 0.3% despite other liquidity
        .to.not.emit(token0, 'Transfer')
  ...
  describe('#collect', () => {
    ...
    it('works with multiple LPs', async () => {
      ...
      expect(tokensOwed0Position0).to.be.eq('133333333333334')
      expect(tokensOwed0Position1).to.be.eq('266666666666668')
    })
  ...
  describe('#feeProtocol', () => {
    ...
    it('fees collected by lp after two swaps should be double one swap', async () => {
      ...
      // 6 bips * 2e18
      expect(token0Fees).to.eq('799999999999998')
      expect(token1Fees).to.eq(0)
    })
    ...
    it('fees collected after two swaps with fee turned on in middle are fees from last swap (not confiscatory)', async () => {
      ...
      expect(token0Fees).to.eq('799999999999999')
      expect(token1Fees).to.eq(0)
    })
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestGlowV3ReentrantCallee')
      ).deploy()) as TestGlowV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestGlowV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestGlowV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestGlowV3SwapPay
