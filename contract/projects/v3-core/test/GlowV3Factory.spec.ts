...
describe('GlowV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: GlowV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as GlowV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as GlowV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [GlowV3Factory, GlowV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('GlowV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('GlowV3Pool')
