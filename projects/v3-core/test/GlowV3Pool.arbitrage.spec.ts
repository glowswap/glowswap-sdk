...
import { GlowV3PoolSwapTest } from '../typechain-types/contracts/test/GlowV3PoolSwapTest'
...
describe('GlowV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('GlowV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as GlowV3PoolSwapTest
