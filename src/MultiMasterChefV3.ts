...
import { GlowV3Token } from './GlowV3Token'
import { GlowV3Pool, type Slot } from './GlowV3Pool'
...
export class MultiMasterChefV3 {
  ...
  private async buildTokenBalance(tokenAddress: Address, owner: Address) {
    const glowToken = new GlowV3Token(this.publicClient, tokenAddress)
    const balance = await glowToken.getTokenBalance(owner)
    return balance
  }
  ...
  public async buildMasterPoolData(pid: bigint, showSlot: boolean = false): Promise<MultiMasterPool | undefined> {
    ...
    if (showSlot) {
      const [rewarderInfoList, slot] = await Promise.all([this.getPoolRewarderInfo(pool.v3Pool), new GlowV3Pool(this.publicClient, pool.v3Pool).getSlot0()])
  ...
  private async buildToken(tokenAddress: Address) {
    ...
    if (token === undefined) {
      const glowToken = new GlowV3Token(this.publicClient, tokenAddress)
      token = await glowToken.readToken()
